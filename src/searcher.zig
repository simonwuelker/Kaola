...
pub fn search(state: GameState, depth: u8, allocator: Allocator) !Move {
    _ = state;
    _ = depth;
    _ = allocator;
    // return try alpha_beta_search(Color.white, state, depth, allocator);
    unreachable;
}
...
// }
//
// fn alpha_beta_search(state: GameState, depth: u8, allocator: Allocator) Allocator.Error!Move {
//     var move_list = ArrayList(Move).init(allocator);
//     defer move_list.deinit();
//
//     try generate_moves(state, &move_list);
//
...
//         const new_state = state.make_move(move_to_consider);
//         const score = try min_value(active_color, new_state, depth - 1, allocator, MIN_SCORE, MAX_SCORE);
//
...
// }
//
// fn max_value(comptime player: Color, state: GameState, depth: u8, allocator: Allocator, alpha_: i32, beta_: i32) Allocator.Error!i32 {
//     if (depth == 0) {
...
//         log_move(move_to_consider, depth, allocator);
//
//         state.make_move(active_color, move_to_consider);
//         score = @maximum(score, try min_value(player, state, depth - 1, allocator, alpha, beta));
//
//         if (score >= beta) {
//             return score;
//         }
